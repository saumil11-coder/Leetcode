	...
	for(int i = 0; i < n; i++) {
		...
		if (length == 1) {
			if (wordMap.containsKey("")) {
				ans.add(List.of(i, wordMap.get("")));
				ans.add(List.of(wordMap.get(""), i));
			} 
		} 
			...
			if (wordMap.containsKey(reverse) && wordMap.get(reverse) != i)
				ans.add(List.of(i, wordMap.get(reverse)));
			...
			for (Integer k : set) {
				...
				if (isPalindrome(reverse, 0, length - 1 - k)) {
					...
					if (wordMap.containsKey(s1))
						ans.add(List.of(i, wordMap.get(s1)));
				}
				...
				if (isPalindrome(reverse, k, length - 1)) {
					...
					if (wordMap.containsKey(s2))
						ans.add(List.of(wordMap.get(s2), i));
				}
			}
		}
	}
